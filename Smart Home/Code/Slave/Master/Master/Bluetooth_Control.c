...
void User_Remoted_Login()
{
	...
	else								// if not
	{
		...
		if (wrong_pass_count == 3)				// if wrong pass counter equal to 3
		{
			...
			LCD_vidMoveCursor(4,10);			// LCD move cursor to fourth line
...
void control_Remoted_Rooms()
{
	...
	while(u8ReturnedValue!='0')					// while loop work if choice not equal 0
	{
		...
		switch(u8ReturnedValue)					// switch choice
		{
			...
			break;						// break
			case 'z' :					// if choice equal to z
			LCD_vidMoveCursor(1,1);				// LCD move cursor to first line
			...
			USART_vidEnableTXInterrupt();			// enable TX interrupt (finish transmit)
			LCD_vidMoveCursor(1,8);				// LCD move cursor to first line	
			...
			USART_vidEnableTXInterrupt();			// enable TX interrupt (finish transmit)
			LCD_vidMoveCursor(2,1);				// LCD move cursor to second line
			...
			USART_vidEnableTXInterrupt();			// enable TX interrupt (finish transmit)
			LCD_vidMoveCursor(2,8);				// LCD move cursor to second line
			...
			USART_vidEnableTXInterrupt();			// enable TX interrupt (finish transmit)
			LCD_vidMoveCursor(3,1);				// LCD move cursor to third line
			...
			USART_vidEnableTXInterrupt();			// enable TX interrupt (finish transmit)
			LCD_vidMoveCursor(3,8);				// LCD move cursor to third line
