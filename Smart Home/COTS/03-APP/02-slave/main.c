...
int main(void)
{
	ADC_vidInit();//initialize the ADC of the micro controller
	// timer0_initializeCTC();//Initialize the timer zero of the micro controller
	SPI_vidInit();//initialize the SPI as a slave
	
	/* initialization of output pins of connected leds or devices */
	LED_vidInit(AIR_COND_PORT,AIR_COND_PIN);
	LED_vidInit(TV_PORT,TV_PIN);
	LED_vidInit(ROOM1_PORT,ROOM1_PIN);
	LED_vidInit(ROOM2_PORT,ROOM2_PIN);
	LED_vidInit(ROOM3_PORT,ROOM3_PIN);
	LED_vidInit(ROOM4_PORT,ROOM4_PIN);
	...
	while(1)
	{
		request = SPI_u8SendReceive(DEFAULT_ACK);//wait for the master to start the transmitting
		...
		switch (request)
		{
			...
			SPI_u8SendReceive(response);//response to the transmitter with the status
			...
			/*********************************   TURN ON COMMANDS ********************************/
			case ROOM1_TURN_ON:
			LED_vidTurnOn(ROOM1_PORT,ROOM1_PIN);//turn on the led of room 1
			break;//break the switch case
			case ROOM2_TURN_ON:
			LED_vidTurnOn(ROOM1_PORT,ROOM2_PIN);//turn on the led of room 2
			break;//break the switch case
			case ROOM3_TURN_ON:
			LED_vidTurnOn(ROOM3_PORT,ROOM3_PIN);//turn on the led of room 3
			break;//break the switch case
			case ROOM4_TURN_ON:
			LED_vidTurnOn(ROOM4_PORT,ROOM4_PIN);//turn on the led of room 4
			break;//break the switch case
			case AIR_COND_TURN_ON:
		//	timer0_initializeCTC();
			LED_vidTurnOn(AIR_COND_PORT,AIR_COND_PIN);//turn on the led of air conditioning
			break;//break the switch case
			case TV_TURN_ON:
			LED_vidTurnOn(TV_PORT,TV_PIN);//turn on the led of the TV
			break;//break the switch case
			
			/*********************************   TURN OFF COMMANDS ********************************/
			case ROOM1_TURN_OFF:
			LED_vidTurnOff(ROOM1_PORT,ROOM1_PIN);//turn off the led of room 1
			break;//break the switch case
			case ROOM2_TURN_OFF:
			LED_vidTurnOff(ROOM2_PORT,ROOM2_PIN);//turn off the led of room 2
			break;//break the switch case
			case ROOM3_TURN_OFF:
			LED_vidTurnOff(ROOM3_PORT,ROOM3_PIN);//turn off the led of room 3
			break;//break the switch case
			case ROOM4_TURN_OFF:
			LED_vidTurnOff(ROOM4_PORT,ROOM4_PIN);//turn off the led of room 4
			break;//break the switch case
			case AIR_COND_TURN_OFF:
			timer0_stop();
			LED_vidTurnOff(AIR_COND_PORT,AIR_COND_PIN);//turn off the led of air conditioning
			break;//break the switch case
			case TV_TURN_OFF:
			LED_vidTurnOff(TV_PORT,TV_PIN);//turn off the led of the TV
			break;//break the switch case
			
			/*********************************   Set temperature   ********************************/
			case SET_TEMPERATURE:
			required_temperature = SPI_u8SendReceive(DEFAULT_ACK);//get the temperature from the master and store the temperature in required temperature 
			break;//break the switch case
		}
	}
}
...
ISR(TIMER0_COMP_vect)
{
	...
	if(counter>=10)//do that code every 10 ticks 
	{
		...
		{
			LED_vidTurnOn(AIR_COND_PORT,AIR_COND_PIN);//turn on the led of the air conditioning
		...
		{
			LED_vidTurnOff(AIR_COND_PORT,AIR_COND_PIN);//turn off the led of the air conditioning
